...
## Short Description
Select or capture study questions and get a fast second-opinion tutor inline.

## Full Description
Solv helps students and self-learners check their work without switching tabs. Select question text on any page, or capture a region with a diagram or handwritten prompt, and Solv shows a compact second-opinion answer overlay on the page. You can continue the conversation in the side panel, copy the answer, regenerate it, or run an independent verification pass when confidence is low.
...
- No Solv backend, no remote extension code, and no advertising or sale of user data.

## Single Purpose
Solv's single purpose is to help users get a second-opinion tutor response for study questions they explicitly select, type, paste, or capture.
...
Solv uses broad content-script matching so the user can invoke the same selected-text and screenshot workflow on ordinary web pages they choose. It does not automatically read pages in the background; solve actions are user initiated.
...
Solv stores settings and optional API keys locally in Chrome storage. When a user asks Solv to answer a question, the selected text, typed prompt, pasted image, or captured screenshot is sent only to the provider the user selected, or to local/on-device AI if selected. Solv does not operate a server, sell user data, use data for advertising, load remote executable code, or allow human review of user content.
